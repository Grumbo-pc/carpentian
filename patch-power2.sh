...
old = """            searchBox.add_actor(this.searchEntry);
            ...
            let powerButtons = [
                { icon: 'system-shutdown-symbolic', tooltip: _('Log Out'), cmd: 'cinnamon-session-logout --logout --no-prompt' },
...
new = """            searchBox.add_actor(this.searchEntry);
            ...
            let powerBtn = new St.Button({
                style: 'padding: 6px 10px; border-radius: 6px; background: rgba(255, 255, 255, 0.08); border: 1px solid rgba(255, 255, 255, 0.15); margin-left: 8px;'
            });
            ...
            powerBtn.add_actor(powerIcon);
            powerBtn.connect('enter-event', () => {
                powerBtn.set_style('padding: 6px 10px; border-radius: 6px; background: rgba(255, 255, 255, 0.18); border: 1px solid rgba(255, 255, 255, 0.3); margin-left: 8px;');
            });
            powerBtn.connect('leave-event', () => {
                powerBtn.set_style('padding: 6px 10px; border-radius: 6px; background: rgba(255, 255, 255, 0.08); border: 1px solid rgba(255, 255, 255, 0.15); margin-left: 8px;');
            });
